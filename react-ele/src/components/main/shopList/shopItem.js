...
class ShopItem extends Component {
  ...
  render () {
    let shopInfo = this.props.shopDetail,
        ...
        deliveryDIV = shopInfo.delivery_mode ? (
          <div className="delivery">
            <div className="delivery_icon">蜂鸟专送</div>
          </div>
        ) : '',
    ...
    activityList = activityArray.map((item, index) => {
      return (
        <div key={index} className="act_row" style={{ display: (index > 1 && !this.state.showActivities) ? 'none' : '' }}>
          <span className="act_icon" style={{ color: '#fff', backgroundColor: `#${item.icon_color}` }}>
            <span className="act_icon_text">{ item.icon_name }</span>
          </span>
