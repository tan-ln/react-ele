...
class  getAddress extends Component {
  ...
  componentDidMount () {
    ...
    const addressList = requestGetReceiveAddress()
    this.setState({
      addressList
    })
    ...
    return (
      <div className="getAdderss">
        <div style={{ display: this.state.showCityList ? 'none' : '', height: '100%', overflowY: 'auto' }}>
          ...
          <section className="delivery_address" style={{ display: hiddenSearchResult }}>
            <h4>收货地址</h4>
            <div className="detail_address_group">
              {
                this.state.addressList.map((user, key) => {
                  return (
                    <div key={key}  className="detail_address_cell">
                      <p>
                        <span>{user.name}</span>
                        <span>{user.sex === 1 ? "先生" : "女士"}</span>
